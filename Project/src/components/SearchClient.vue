<template>
  <q-input
    ...
    color="yellow"
    v-model="searchBox"
    label="Digite o Nome do cliente para verificar"
    debounce="300"
    class="custom-input"
  >
        ...
        >
          ...
          >
            <q-item-section>
              ...
              <q-item-label v-if="item.authpass_history">
                <strong class="data-highlight">Histórico de quedas do cliente:</strong>
                <q-list bordered>
                  <q-item
                    v-for="(history, idx) in item.authpass_history"
                    :key="idx"
                  >
                    <q-item-section>
                      <q-item-label>
                        <strong class="data-highlight">Tempo de Autenticação: </strong>
                        <span class="data-highlight">{{ history.authpass_time }}</span>
                      </q-item-label>
                      <q-item-label>
                        <strong class="data-highlight">Tempo Offline: </strong>
                        <span class="data-highlight">{{ history.offline_time }}</span>
                      </q-item-label>
                      <q-item-label>
                        <strong class="data-highlight">Causa: </strong>
                        <span class="data-highlight">{{ history.cause }}</span>
                      </q-item-label>
                    </q-item-section>
                  </q-item>
                </q-list>
  ...
  setup() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const url1 = `${import.meta.env.VITE_API_URL_GET}?client=${encodeURIComponent(
  searchBox.value
)}&user=${encodeURIComponent(user.username)}`;

        const url2 = import.meta.env.VITE_API_URL_POST;
        const postData = [
          {
